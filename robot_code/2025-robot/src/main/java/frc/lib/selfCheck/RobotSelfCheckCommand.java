...
public class RobotSelfCheckCommand extends SequentialCommandGroup {
  public RobotSelfCheckCommand(Command onPassCommand, Command onFailedCommand,
      CheckableSubsystem... subsystems) {
    addRequirements(subsystems);
    for (CheckableSubsystem subsystem : subsystems) {
      var commands = subsystem.getCheckCommands();
      for (CheckCommand command : commands) {
        addCommands(
            Commands.print(command.getDescription()).andThen(command).finallyDo((interrupted) -> {
              if (interrupted) {
                anyFailed = true;
